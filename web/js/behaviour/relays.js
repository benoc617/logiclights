// Relay circuits: coils and contacts only, no semiconductors.
//
// These are hand-placed — every device at a chosen coordinate, every wire
// routed by eye — and they should stay that way. Their layouts are the
// explanation, not just a way to fit devices on a canvas. The composed
// machines in cmos.js and nmos.js are the ones assembled from modules.
//
// Conduction is undirected, so these topologies are designed sneak-path
...
export const relays = {
  relay101: { build: buildRelay101 },
  buzzer: { build: buildBuzzer },
  osc: { build: buildOscillator },
  not: { build: buildNot },
  and: { build: buildAnd },
  or: { build: buildOr },
  xor: { build: buildXor },
  nand: { build: buildNand },
  nor: { build: buildNor },
  dec24: { build: buildDecoder },
  srlatch: { build: buildSrLatch },
  dlatch: { build: buildDLatch },
  reg4: { build: buildRegister4 },
  halfadd: {
    build: buildHalfAdder,
