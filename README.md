...
- **A CPU being built, one idea at a time** — nine machines under
  ...
  sequencing, then an accumulator that changes state, a program counter
  that can be jumped to, the 4004's real `ADD r` datapath, conditional
  jumps with the full condition mask, two-byte fetch, `SUB` sharing the
  adder with `ADD`, and the thirteen-instruction accumulator group. Give
  one a clock and it runs a program on its own — the ROM is disassembled
  beside it with
